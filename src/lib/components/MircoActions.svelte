<script>
  ...
  let invokeShareFallback = false;

  function sharepost(event) {
    if (!navigator.canShare()) {
      event.stopPropagation();
      invokeShareFallback = !invokeShareFallback;
      return;
    }
...
<div>
  ...
  {#if invokeShareFallback}
    <article
      transition:slide
      use:clickOutside={() => (invokeShareFallback = false)}
    >
      <hgroup>
        <p>{shareData.text}</p>
        <p>
